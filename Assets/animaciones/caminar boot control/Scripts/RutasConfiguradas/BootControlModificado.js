...
public var Ruta : cRuta; // MODIFICACION
public var TipoRuta : int = 1;
public var Margen : float = 0.2;
public var Velocidad : float = 1;
...
function FixedUpdate () {
		...
		if (Ruta != null) {
			...
			destino = Ruta.Posicion(distancia,TipoRuta);
			destino.y = transform.position.y;
						
			transform.LookAt(destino);
			
			if (Vector3.Distance(transform.position,destino)<Margen) distancia += Time.deltaTime*Velocidad;
			
			if ((TipoRuta == 0) && (v > 0)) {
			
				if (distancia >= Ruta.GetdRuta()) v = 0;
